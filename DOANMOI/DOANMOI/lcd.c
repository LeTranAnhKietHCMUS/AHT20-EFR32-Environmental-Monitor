...
sl_status_t lcd_tx(const struct sl_memlcd_t *device,
                   const void *data,
                   unsigned int row_start,
                   unsigned int row_count)
{
  if (!device || !data || row_count == 0) {
    return SL_STATUS_INVALID_PARAMETER;
  }

  // Validate row parameters for 128x128 display
  if (row_start >= 128 || (row_start + row_count) > 128) {
    return SL_STATUS_INVALID_PARAMETER;
  }
  ...
  uint16_t bytes_per_row = device->width / 8;  // 128 pixels / 8 bits per byte
  uint8_t transmit_buffer[130];  // Max 2 (mode+address) + 128 (data) bytes

  // ========== DATA UPDATE MODE 1 LINE ==========
  // Update rows one by one
  for (unsigned int row = 0; row < row_count; row++) {
    uint8_t current_row = row_start + row + 1;  // Row addresses are 1-128

    // Build transmission buffer
    uint8_t buffer_idx = 0;

    // Byte 1: Mode/Command
    // Format: M2 M1 M0 | 0 0 0 0 0
    // M0 = 1 (Data Update Mode)
    // M1 = 0 (Don't care)
    // M2 = 0 (Not all clear)
    mode_byte = 0x01;  // Binary: 00000001
    transmit_buffer[buffer_idx++] = mode_byte;

    // Byte 2: Gate Line Address (Row number)
    // AG0-AG7: Row address (1 to 128)
    transmit_buffer[buffer_idx++] = current_row;

    // Bytes 3 to 18: Image data for this row (16 bytes)
    // Each byte represents 8 pixels (1 bit per pixel)
    // 1 = White pixel, 0 = Black pixel
    uint16_t src_offset = (row_start + row) * bytes_per_row;
    memcpy(&transmit_buffer[buffer_idx],
           &pixel_data[src_offset],
           bytes_per_row);
    buffer_idx += bytes_per_row;

    // Byte 19: Dummy byte (for protocol completion)
    transmit_buffer[buffer_idx++] = 0x00;

    // Total bytes to send: 1 (mode) + 1 (address) + 16 (data) + 1 (dummy) = 19

    // ========== SPI TRANSMISSION ==========
    // Set CS pin high (chip select active)
    GPIO_PinOutSet(SL_MEMLCD_SPI_CS_PORT, SL_MEMLCD_SPI_CS_PIN);

    // Wait for CS setup time (typically 6 µs minimum)
    sl_udelay_wait(device->setup_us);

    // Transmit data via SPI
    status = sli_memlcd_spi_tx(&spi_handle, transmit_buffer, buffer_idx);
    ...
    // Wait for CS hold time (typically 2 µs minimum)
    sl_udelay_wait(device->hold_us);

    // Clear CS pin (chip select inactive)
    GPIO_PinOutClear(SL_MEMLCD_SPI_CS_PORT, SL_MEMLCD_SPI_CS_PIN);
  }
